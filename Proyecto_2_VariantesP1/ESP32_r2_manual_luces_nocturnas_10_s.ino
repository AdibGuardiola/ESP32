...
bool manualOverrideR2 = false; // Variable para saber si el relé 2 fue encendido manualmente
...
void mensajesNuevos(int numerosMensajes)
{
  for (int i = 0; i < numerosMensajes; i++)
  {
    ...
    {
      estadoLed13 = 1;
      manualOverrideR2 = true; // Marcar que se encendió manualmente
      digitalWrite(led13, LOW);      
      bot.sendMessage(chat_id, "Relé2 encendido manualmente", "");
    }

    if (text == "R2off")
    {
      estadoLed13 = 0;
      manualOverrideR2 = false; // Marcar que ya no está en modo manual
...
void loop()
{
  if (millis() - tiempoAnterior > tiempo)
  {
    ...
    // Si el LDR no detecta luz y el relé 2 está apagado y no fue encendido manualmente
    if (estadoLDR == HIGH && estadoLed13 == 0 && !manualOverrideR2) 
    {
    ...
    // Si el LDR detecta luz y el relé 2 está encendido y no fue encendido manualmente
    else if (estadoLDR == LOW && estadoLed13 == 1 && !manualOverrideR2) 
    {
    ...
    // Verificar si han pasado 10 segundos desde que se encendió el relé 2
    if (estadoLed13 == 1 && (millis() - tiempoEncendidoR2 >= 10000) && !manualOverrideR2)
    {
      estadoLed13 = 0; // Apagar el relé 2 automáticamente después de 10 segundos si no fue encendido manualmente
      digitalWrite(led13, HIGH);
      bot.sendMessage(ID_Chat, "El relé 2 se ha apagado automáticamente tras 10 segundos.", "");
    }

    // Escaneo de mensajes de Telegram
